...
export function CollectionGroup({ group, onSelect, onAddSongs = () => {}, onDuplicate = () => {}, onGroupCheckboxChange = () => {}, onAlbum = () => {} }) {
        ...
        {!editing && !isExportMode && (
          <>
            ...
            {!isSpecial && (
              <button
                type="button"
                title={`Create album from ${group.name}`}
                onClick={e => { e.stopPropagation(); onAlbum(group) }}
                aria-label={`Create album from ${group.name}`}
